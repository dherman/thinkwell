...
```typescript
import { open, Agent, Session, schemaOf } from "thinkwell";
```

| Export | Description |
|--------|-------------|
| `open()` | Main entry point — opens a connection to a named agent |
| `Agent` | The agent class returned by `open()` |
| `Session` | Manages multi-turn conversations with persistent context |
| `ThinkBuilder` | Fluent API for building prompts with tools |
| `schemaOf()` | Helper for creating schema providers from JSON Schema objects |

See [Named Agents](/api/connectors) for the list of supported agent names.
...
- [Agent](/api/agent) - Opening agents and managing sessions
- [ThinkBuilder](/api/think-builder) - Building prompts with the fluent API
- [Named Agents](/api/connectors) - Available agent names
